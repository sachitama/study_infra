...
resource "aws_security_group" "mng-sg" {
  name        = "${var.project}-${var.environment}-mng-sg"
  description = "mng server role security group"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_security_group_rule" "mng_in_ssh" {
  security_group_id = aws_security_group.mng-sg.id
  type              = "ingress"
  protocol          = "tcp"
  from_port         = 22
  to_port           = 22
...
resource "aws_security_group_rule" "mng_in_tcp3000" {
  security_group_id = aws_security_group.mng-sg.id
  type              = "ingress"
  protocol          = "tcp"
  from_port         = 3000
...
resource "aws_security_group_rule" "mng_out_http" {
  security_group_id = aws_security_group.mng-sg.id
  type              = "egress"
  protocol          = "tcp"
  from_port         = 80
...
resource "aws_security_group_rule" "mng_out_https" {
  security_group_id = aws_security_group.mng-sg.id
  type              = "egress"
  protocol          = "tcp"
  from_port         = 443
